...
class LogDiscoveryService
{
    ...
    public function addCustomPath(string $path): bool
    {
        ...
        // It's important to merge config paths here to prevent config-only paths
        // from being lost during persistent storage overwrite or app restarts.
        $pathsToCache = array_values(array_unique(array_merge($configPaths, $cachePaths)));
    ...
    public function getCustomPaths(): array
    {
        if (!cache()->has('ids.custom_log_paths')) {
            try {
                ...
                Log::error('Failed to migrate custom log paths: ' . $e->getMessage());
