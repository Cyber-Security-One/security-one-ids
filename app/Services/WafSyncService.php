    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                $cleanUser = '';
                if (!empty($user) && preg_match('/^[a-zA-Z0-9._-]+$/', $user)) {
                    $cleanUser = $user;
                }
                ...
                if (!empty($cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    try {
                        ...
                        if ($process1->isTimedOut()) {
                            file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser} timed out\n", FILE_APPEND);
                        }
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser}: code={$returnCode1}, output={$outputStr}\n", FILE_APPEND);
                    } catch (\Exception $e) {
                        $returnCode1 = 1;
                        if (isset($process1) && $process1->isTimedOut()) {
                    ...
                    if ($returnCode1 !== 0) {
                        ...
                        try {
                            ...
                            if ($process2->isTimedOut()) {
                                file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$cleanUser} timed out\n", FILE_APPEND);
                            }
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$cleanUser}: code={$returnCode2}\n", FILE_APPEND);
                        } catch (\Exception $e) {
                            $returnCode2 = 1;
                            if (isset($process2) && $process2->isTimedOut()) {
                                file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$cleanUser} timed out: " . $e->getMessage() . "\n", FILE_APPEND);
                            } else {
                    ...
                    if ($returnCode1 !== 0 && $returnCode2 !== 0) {
                        ...
                        try {
                            ...
                            if ($process3->isTimedOut()) {
                                file_put_contents($logFile, "[{$timestamp}] dscl set impossible password timed out\n", FILE_APPEND);
                            }
                            file_put_contents($logFile, "[{$timestamp}] dscl set impossible password: code={$returnCode3}\n", FILE_APPEND);
                        } catch (\Exception $e) {
                            $returnCode3 = 1;
                            if (isset($process3) && $process3->isTimedOut()) {
                                file_put_contents($logFile, "[{$timestamp}] dscl set impossible password timed out: " . $e->getMessage() . "\n", FILE_APPEND);
                            } else {
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    $user = trim($user);
                    if (empty($user) || !preg_match('/^[a-zA-Z0-9._-]+$/', $user)) continue;
