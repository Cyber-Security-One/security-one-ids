    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                file_put_contents($logFile, "[{$timestamp}] Console user: {$logUser}\n", FILE_APPEND);
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (empty($user) || !preg_match('/^[a-zA-Z0-9._-]+$/', $user)) {
                        $logUser = preg_replace('/[\r\n]+/', ' ', $user);
                        Log::error('Invalid user for enable login: ' . $logUser);
                        file_put_contents($logFile, "[{$timestamp}] Invalid user for enable login: {$logUser}\n", FILE_APPEND);
                        continue;
                    }
