...
class WafSyncService
{
    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                // Validate username to prevent injection, checking against the list of valid users
                if ($consoleUser && $this->isValidMacUser($consoleUser) && $consoleUser !== 'root' && $consoleUser !== '_mbsetupuser') {
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    $user = trim($user);
                    if (!$user || !$this->isValidMacUser($user)) continue;
    ...
    /**
     * Check if a given macOS username is valid by querying the system
     */
