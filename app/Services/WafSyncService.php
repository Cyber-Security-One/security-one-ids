                    ...
                    if (!$loginDisabled) {
                        ...
                        if (!$loginDisabled) {
                            ...
                            try {
                                ...
                                $process3 = Process::timeout(60)->run(['sudo', 'pwpolicy', '-u', $cleanUser, 'sethash', $randomHash]);
                                if ($process3->successful()) {
                                    $loginDisabled = true;
                                }
                                $returnCode3 = $process3->exitCode();
                                file_put_contents($logFile, "[{$timestamp}] pwpolicy sethash random: code={$returnCode3}\n", FILE_APPEND);
                            } catch (\Exception $e) {
                                file_put_contents($logFile, "[{$timestamp}] pwpolicy sethash random exception: " . $e->getMessage() . "\n", FILE_APPEND);
                            }
                        }
                    }
                    
                    if (!$loginDisabled) {
                        throw new \Exception("Failed to disable user {$cleanUser} via all available methods.");
                    }
                } else {
