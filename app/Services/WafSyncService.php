...
$logConsoleUser = str_replace(["\r", "\n"], ['\\r', '\\n'], $consoleUser);
                file_put_contents($logFile, "[{$timestamp}] Console user: {$logConsoleUser}\n", FILE_APPEND);
...
file_put_contents($logFile, "[{$timestamp}] dscl disable user {$logConsoleUser}: code={$returnCode}, output=" . implode(" ", $output) . "\n", FILE_APPEND);
...
file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$logConsoleUser}: code={$returnCode}\n", FILE_APPEND);
                    }

                    if ($returnCode !== 0) {
                        // Method 3: Set an impossible password hash
                        exec("sudo dscl . -passwd /Users/{$safeConsoleUser} '*' 2>&1", $output, $returnCode);
file_put_contents($logFile, "[{$timestamp}] dscl set impossible password for {$logConsoleUser}: code={$returnCode}\n", FILE_APPEND);
                    }
                } else {
...
$safeUser = escapeshellarg($user);
                    ...
                    file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$logUser}: code={$returnCode}\n", FILE_APPEND);

                    // Re-enable with pwpolicy
                    exec("sudo pwpolicy -u {$safeUser} enableuser 2>&1", $output, $returnCode);
                    file_put_contents($logFile, "[{$timestamp}] pwpolicy enable user {$logUser}: code={$returnCode}\n", FILE_APPEND);
                }
