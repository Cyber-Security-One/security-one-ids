        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && preg_match('/^[a-zA-Z0-9._-]+$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    $method1Failed = !$dsclDisableExecuted || ($dsclDisableResult !== 0 && (stripos($outputStr ?? '', 'eDS') !== false || stripos($outputStr ?? '', 'error') !== false));
                    if ($method1Failed) {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    try {
                        ...
                        $outputStr = trim($process1->getOutput() . ' ' . $process1->getErrorOutput());
                        file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$cleanUser}: code={$dsclClearResult}, output={$outputStr}\n", FILE_APPEND);
                    } catch (\Exception $e) {
                    ...
                    $dsclClearSuccess = $dsclClearExecuted && ($dsclClearResult === 0 || (stripos($outputStr ?? '', 'eDS') === false && stripos($outputStr ?? '', 'error') === false));
                    $success = $dsclClearSuccess && ($pwpolicyEnableExecuted && $pwpolicyEnableResult === 0);
                    if (!$success) {
