            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($user && $user !== 'root' && $user !== '_mbsetupuser') {
                    if (!preg_match('/^[a-zA-Z0-9._-]+$/', $user)) {
                        $cleanUserForLog = preg_replace('/[\r\n]+/', ' ', $user);
                        file_put_contents($logFile, "[{$timestamp}] Warning: User '{$cleanUserForLog}' failed regex validation, skipping dscl create\n", FILE_APPEND);
                        $returnCode = 1;
                    } else {
                        // Method 1: Use dscl to disable user account
                        // The correct way is to set AuthenticationAuthority to DisabledUser
                        $process = Process::timeout(60)->run(['sudo', 'dscl', '.', '-create', '/Users/' . $user, 'AuthenticationAuthority', ';DisabledUser;']);
                        $returnCode = $process->exitCode();
                        $outputStr = trim($process->output() . ' ' . $process->errorOutput());
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$user}: code={$returnCode}, output={$outputStr}\n", FILE_APPEND);
                    }
                    
                    if ($returnCode !== 0) {
                        // Method 2: Lock the user's password (they won't be able to login)
                        $process = Process::timeout(60)->run(['sudo', 'pwpolicy', '-u', $user, 'disableuser']);
                        $returnCode = $process->exitCode();
                        file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$user}: code={$returnCode}\n", FILE_APPEND);
                    }
                    
                    if ($returnCode !== 0) {
                        // Method 3: Set an impossible password hash
                        $process = Process::timeout(60)->run(['sudo', 'dscl', '.', '-passwd', '/Users/' . $user, '*']);
                        $returnCode = $process->exitCode();
                        file_put_contents($logFile, "[{$timestamp}] dscl set impossible password {$user}: code={$returnCode}\n", FILE_APPEND);
                    }
                } else {
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (!preg_match('/^[a-zA-Z0-9._-]+$/', $user)) {
                        $cleanUserForLog = preg_replace('/[\r\n]+/', ' ', $user);
                        file_put_contents($logFile, "[{$timestamp}] Warning: User '{$cleanUserForLog}' failed regex validation, skipping dscl clear auth\n", FILE_APPEND);
                        $returnCode = 1;
                    } else {
                        // Remove DisabledUser from AuthenticationAuthority
                        $process = Process::timeout(60)->run(['sudo', 'dscl', '.', '-delete', '/Users/' . $user, 'AuthenticationAuthority']);
                        $returnCode = $process->exitCode();
                        file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$user}: code={$returnCode}\n", FILE_APPEND);
                    }
                    
                    // Re-enable with pwpolicy  
                    $process = Process::timeout(60)->run(['sudo', 'pwpolicy', '-u', $user, 'enableuser']);
                    $returnCode = $process->exitCode();
