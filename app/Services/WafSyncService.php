    ...
    private function handleEnableLogin(): void
    {
        try {
            ...
            if (empty($failedUsers ?? [])) {
                ...
                echo "⚠️ Login enabled with partial failures\n";
                Log::warning('Login enable completed with partial failures', ['failed_users' => $failedUsers]);
                file_put_contents($logFile, "[{$timestamp}] Login enable completed with partial failures: " . implode(', ', $failedUsers) . "\n", FILE_APPEND);
            }
