        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if (!empty($user) && !preg_match('/^[a-zA-Z0-9._-]+$/', $user)) {
                    Log::error("Invalid characters in macOS username: {$user}");
                    file_put_contents($logFile, "[{$timestamp}] Invalid characters in macOS username: {$user}\n", FILE_APPEND);
                    $cleanUser = '';
                } else {
                    $cleanUser = $user;
                }
                ...
                if (!empty($cleanUser) && $cleanUser !== 'root' && $cleanUser !== '_mbsetupuser') {
                    $returnCode1 = 0;
                    $returnCode2 = 0;
