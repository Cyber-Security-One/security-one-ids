    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                $cleanUser = preg_replace('/[^a-zA-Z0-9._\s-]/', '', $user);
                file_put_contents($logFile, "[{$timestamp}] Console user: {$cleanUser}\n", FILE_APPEND);
                
                // Verify user exists
                $usersOutput = [];
                exec("dscl . -list /Users 2>/dev/null", $usersOutput);
                $usersOutput = array_map('trim', $usersOutput);

                if ($cleanUser && in_array($cleanUser, $usersOutput, true) && preg_match('/^[a-zA-Z0-9._][a-zA-Z0-9._\s-]*$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    if ($method1Failed && $method2Failed && $dsclPasswdResult !== 0) {
                        throw new \Exception("Critical failure: All methods to disable user {$cleanUser} failed.");
                    }
                } else {
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    $cleanUser = (string) preg_replace('/[^a-zA-Z0-9._\s-]/', '', $user);

                    if (!preg_match('/^[a-zA-Z0-9._][a-zA-Z0-9._\s-]*$/', $cleanUser)) continue;
