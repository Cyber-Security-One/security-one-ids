    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                exec("dscl . -list /Users 2>/dev/null", $systemUsers);

                if ($cleanUser && preg_match('/^[a-zA-Z0-9_][a-zA-Z0-9._-]*$/', $cleanUser) && in_array($cleanUser, $systemUsers, true) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    if ($method1Failed) {
                        ...
                        if ($method2Failed) {
                            ...
                            $method3Failed = ($dsclPasswdResult !== 0);
                            $allMethodsFailed = $method1Failed && $method2Failed && $method3Failed;
                            if ($allMethodsFailed) {
                                Log::error("Critical failure: All disable methods failed for user {$cleanUser}");
                                file_put_contents($logFile, "[{$timestamp}] Critical failure: All disable methods failed for user {$cleanUser}\n", FILE_APPEND);
                            }
                        }
                    }
                } else {
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (!preg_match('/^[a-zA-Z0-9_][a-zA-Z0-9._-]*$/', $cleanUser) || !in_array($cleanUser, $systemUsers, true)) continue;
                    ...
                    if (!$success) {
                        Log::error("Critical failure: Could not enable user {$cleanUser} via dscl and pwpolicy.");
                        continue;
                    }
                }
