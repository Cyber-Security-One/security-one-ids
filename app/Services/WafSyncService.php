            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($consoleUser && $consoleUser !== 'root' && $consoleUser !== '_mbsetupuser') {
                    ...
                    if (!preg_match('/^\S+$/', $cleanUser)) {
                        file_put_contents($logFile, "[{$timestamp}] Skipped due to validation: invalid username format '{$cleanUser}'\n", FILE_APPEND);
                    } else {
                        // Method 1: Use dscl to disable user account
                        // The correct way is to set AuthenticationAuthority to DisabledUser
                        $output = [];
                        $returnCode = 0;
                        $escapedUser = escapeshellarg($cleanUser);
                        ...
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser}: {$status}, code={$returnCode}, output=" . implode(" ", $output) . "\n", FILE_APPEND);
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    $status = $returnCode === 0 ? 'success' : 'failed execution';
                    file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$cleanUser}: {$status}, code={$returnCode}\n", FILE_APPEND);
                    ...
                    $status = $returnCode === 0 ? 'success' : 'failed execution';
                    file_put_contents($logFile, "[{$timestamp}] pwpolicy enable user {$cleanUser}: {$status}, code={$returnCode}\n", FILE_APPEND);
                }
