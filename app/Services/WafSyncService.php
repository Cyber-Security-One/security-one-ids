        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($consoleUser && $consoleUser !== 'root' && $consoleUser !== '_mbsetupuser') {
                    ...
                    $output = [];
                    exec("sudo dscl . -create /Users/" . escapeshellarg($safeConsoleUser) . " AuthenticationAuthority ';DisabledUser;' 2>&1", $output, $returnCode);
                    ...
                    if ($returnCode !== 0) {
                        ...
                        exec("sudo pwpolicy -u " . escapeshellarg($safeConsoleUser) . " disableuser 2>&1", $output, $returnCode);
                        file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user: code={$returnCode}\n", FILE_APPEND);
                    }
                    
                    if ($returnCode !== 0) {
                        // Method 3: Set an impossible password hash
                        exec("sudo dscl . -passwd /Users/" . escapeshellarg($safeConsoleUser) . " '*' 2>&1", $output, $returnCode);
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    exec("sudo dscl . -delete /Users/" . escapeshellarg($safeUser) . " AuthenticationAuthority 2>&1", $output, $returnCode);
                    file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$safeUser}: code={$returnCode}\n", FILE_APPEND);
                    
                    // Re-enable with pwpolicy  
                    exec("sudo pwpolicy -u " . escapeshellarg($safeUser) . " enableuser 2>&1", $output, $returnCode);
