            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($consoleUser && $consoleUser !== 'root' && $consoleUser !== '_mbsetupuser') {
                    if (!preg_match('/^[a-zA-Z0-9_]+$/', $consoleUser)) {
                        ...
                        Log::warning('Invalid console username format detected, aborting operation for user: ' . $sanitizedUser);
                        file_put_contents($logFile, "[{$timestamp}] Invalid console username format detected, aborting operation for user: {$sanitizedUser}\n", FILE_APPEND);
                    } else {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (!preg_match('/^[a-zA-Z0-9_]+$/', $user)) {
                        $sanitizedUser = (string) preg_replace('/[\r\n]+/', ' ', $user);
                        Log::warning('Invalid username format detected, skipping user: ' . $sanitizedUser);
                        file_put_contents($logFile, "[{$timestamp}] Invalid username format detected, skipping user: {$sanitizedUser}\n", FILE_APPEND);
                        continue;
                    }
