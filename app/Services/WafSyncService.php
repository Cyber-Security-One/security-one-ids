            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && preg_match('/^[a-zA-Z0-9_]+$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    try {
                        ...
                        $returnCode1 = 1;
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser} error: " . get_class($e) . ': ' . $e->getMessage() . "\n", FILE_APPEND);
                    }
                    ...
                    if ($returnCode1 !== 0) {
                        ...
                        try {
                            ...
                            $returnCode2 = 1;
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$cleanUser} error: " . get_class($e) . ': ' . $e->getMessage() . "\n", FILE_APPEND);
                        }
                    }
                    ...
                    if ($returnCode2 !== -1 && $returnCode2 !== 0) {
                        ...
                        try {
                            ...
                            $returnCode3 = 1;
                            file_put_contents($logFile, "[{$timestamp}] dscl set impossible password error: " . get_class($e) . ': ' . $e->getMessage() . "\n", FILE_APPEND);
                        }
                    }
                ...
                foreach ($usersOutput as $user) {
                    ...
                    while ($returnCode1 !== 0 && $attempts1 < 3) {
                        ...
                        try {
                            ...
                            $hasException = true;
                            $returnCode1 = 1;
                            file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$cleanUser} error: " . get_class($e) . ': ' . $e->getMessage() . "\n", FILE_APPEND);
                        }
                    ...
                    while ($returnCode2 !== 0 && $attempts2 < 3) {
                        ...
                        try {
                            ...
                            $returnCode2 = 1;
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy enable user {$cleanUser} error: " . get_class($e) . ': ' . $e->getMessage() . "\n", FILE_APPEND);
                        }
