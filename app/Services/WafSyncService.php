            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($consoleUser && $consoleUser !== 'root' && $consoleUser !== '_mbsetupuser') {
                    if (!preg_match('/^[a-zA-Z0-9_.-]+$/', $consoleUser)) {
                        ...
                        Log::warning("Invalid console user format for disable login: {$consoleUser}");
                    } else {
                        $safeUserPath = escapeshellarg("/Users/{$consoleUser}");
                        $safeUser = escapeshellarg($consoleUser);

                        // Method 1: Use dscl to disable user account
                        // The correct way is to set AuthenticationAuthority to DisabledUser
                        $output = [];
                        exec("sudo dscl . -create {$safeUserPath} AuthenticationAuthority ';DisabledUser;' 2>&1", $output, $returnCode);
                        ...
                        if ($returnCode !== 0) {
                            ...
                            exec("sudo pwpolicy -u {$safeUser} disableuser 2>&1", $output, $returnCode);
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user: code={$returnCode}\n", FILE_APPEND);
                        }

                        if ($returnCode !== 0) {
                            // Method 3: Set an impossible password hash
                            exec("sudo dscl . -passwd {$safeUserPath} '*' 2>&1", $output, $returnCode);
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (!preg_match('/^[a-zA-Z0-9_.-]+$/', $user)) {
                        ...
                        Log::warning("Invalid user format for enable login: {$user}");
                        continue;
                    }

                    $safeUserPath = escapeshellarg("/Users/{$user}");
                    $safeUser = escapeshellarg($user);

                    // Remove DisabledUser from AuthenticationAuthority
                    exec("sudo dscl . -delete {$safeUserPath} AuthenticationAuthority 2>&1", $output, $returnCode);
