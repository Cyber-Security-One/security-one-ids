            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && preg_match('/^[a-zA-Z0-9._-]+$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    try {
                        ...
                        $process1->run();
                        $returnCode1 = $process1->isSuccessful() ? 0 : ($process1->getExitCode() ?? 1);
                        ...
                        $returnCode1 = 1;
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser} error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES) . "\n", FILE_APPEND);
                    } catch (\Exception $e) {
                        $returnCode1 = 1;
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser} error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES) . "\n", FILE_APPEND);
                    }
                    ...
                    if ($returnCode1 !== 0) {
                        ...
                        try {
                            ...
                            $process2->run();
                            $returnCode2 = $process2->isSuccessful() ? 0 : ($process2->getExitCode() ?? 1);
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$cleanUser}: code={$returnCode2}\n", FILE_APPEND);
                        } catch (\Symfony\Component\Process\Exception\ProcessFailedException $e) {
                            $returnCode2 = 1;
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$cleanUser} error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES) . "\n", FILE_APPEND);
                        } catch (\Exception $e) {
                            $returnCode2 = 1;
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$cleanUser} error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES) . "\n", FILE_APPEND);
                        }
                    ...
                    if ($returnCode1 !== 0 && $returnCode2 !== 0) {
                        ...
                        try {
                            ...
                            $process3->run();
                            $returnCode3 = $process3->isSuccessful() ? 0 : ($process3->getExitCode() ?? 1);
                            file_put_contents($logFile, "[{$timestamp}] dscl set impossible password: code={$returnCode3}\n", FILE_APPEND);
                        } catch (\Symfony\Component\Process\Exception\ProcessFailedException $e) {
                            $returnCode3 = 1;
                            file_put_contents($logFile, "[{$timestamp}] dscl set impossible password error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES) . "\n", FILE_APPEND);
                        } catch (\Exception $e) {
                            $returnCode3 = 1;
                            file_put_contents($logFile, "[{$timestamp}] dscl set impossible password error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES) . "\n", FILE_APPEND);
                        }
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (!preg_match('/^[a-zA-Z0-9._-]+$/', $cleanUser) || $cleanUser === 'daemon' || $cleanUser === 'nobody' || $cleanUser === '_mbsetupuser') continue;
                    ...
                    try {
                        ...
                        $returnCode1 = 1;
                        file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$cleanUser} error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES) . "\n", FILE_APPEND);
                    } catch (\Exception $e) {
                        $returnCode1 = 1;
                        file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$cleanUser} error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES) . "\n", FILE_APPEND);
                    }
                    ...
                    try {
                        ...
                        $returnCode2 = 1;
                        file_put_contents($logFile, "[{$timestamp}] pwpolicy enable user {$cleanUser} error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES) . "\n", FILE_APPEND);
                    } catch (\Exception $e) {
                        $returnCode2 = 1;
                        file_put_contents($logFile, "[{$timestamp}] pwpolicy enable user {$cleanUser} error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES) . "\n", FILE_APPEND);
                    }
