    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && preg_match('/^[a-zA-Z0-9._\s-]+$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    // Method 1: Use dscl to disable user account
                    // The correct way is to set AuthenticationAuthority to DisabledUser
                    $method1Success = false;
                    /** @var int|null $returnCode1 */
                    $returnCode1 = null;
                    try {
                        ...
                        $method1Success = false;
                        $returnCode1 = 1;
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser} process exception: " . $e->getMessage() . "\n", FILE_APPEND);
                    } catch (\Exception $e) {
                        $method1Success = false;
                        $returnCode1 = 1;
                    ...
                    if (!$method1Success) {
                        ...
                        try {
                            ...
                            $method2Success = false;
                            $returnCode2 = 1;
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$cleanUser} process exception: " . $e->getMessage() . "\n", FILE_APPEND);
                        } catch (\Exception $e) {
                            $method2Success = false;
                            $returnCode2 = 1;
                        ...
                        if (!$method2Success) {
                            ...
                            try {
                                ...
                                $method3Success = false;
                                $returnCode3 = 1;
                                file_put_contents($logFile, "[{$timestamp}] chpass set expire immediately process exception: " . $e->getMessage() . "\n", FILE_APPEND);
                            } catch (\Exception $e) {
                                $method3Success = false;
                                $returnCode3 = 1;
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (empty($cleanUser) || !preg_match('/^[a-zA-Z0-9._\s-]+$/', $cleanUser)) {
                    ...
                    try {
                        ...
                        $returnCode1 = 1;
                        file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$cleanUser} process exception: " . $e->getMessage() . "\n", FILE_APPEND);
                    } catch (\Exception $e) {
                        $returnCode1 = 1;
                    ...
                    try {
                        ...
                        $returnCode2 = 1;
                        file_put_contents($logFile, "[{$timestamp}] pwpolicy enable user {$cleanUser} process exception: " . $e->getMessage() . "\n", FILE_APPEND);
                    } catch (\Exception $e) {
                        $returnCode2 = 1;
