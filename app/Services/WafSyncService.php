...
class WafSyncService
{
    ...
    /**
     * Sanitize and validate a macOS username.
     * Strips newlines and ensures the username matches expected patterns, allowing spaces.
     *
     * @param string $user
     * @return string|null Returns the sanitized username if valid, or null if invalid.
     */
    private function sanitizeMacUser(string $user): ?string
    {
        $cleanUser = (string) preg_replace('/[\r\n]+/', '', $user);
        if ($cleanUser && preg_match('/^[a-zA-Z0-9._\s-]+$/', $cleanUser)) {
            return $cleanUser;
        }
        return null;
    }
    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                $cleanUser = $this->sanitizeMacUser($user);
                file_put_contents($logFile, "[{$timestamp}] Console user: {$cleanUser}\n", FILE_APPEND);
                
                if ($cleanUser && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
