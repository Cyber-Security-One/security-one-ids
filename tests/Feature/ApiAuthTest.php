...
class ApiAuthTest extends TestCase
{
    ...
    public function test_auth_logic_rejects_missing_token()
    {
        ...
        // Whitespace bearer token should fall back to valid header
        $this->postJson('/api/rules/update', [], [
            'Authorization' => 'Bearer   ',
            'X-Agent-Token' => 'test-agent-token'
        ])->assertStatus(200);

        // Empty header token should fall back to valid query token
        $this->postJson('/api/rules/update?token=test-agent-token', [], [
            'X-Agent-Token' => ''
        ])->assertStatus(200);
    }

    public function test_auth_logic_returns_503_on_missing_configured_token()
    {
        Config::set('ids.agent_token', null);
        $this->postJson('/api/rules/update?token=any-token', [])->assertStatus(503);
    }
