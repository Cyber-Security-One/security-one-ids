...
class LogDiscoveryServiceTest extends TestCase
{
    ...
    {
        $filename = uniqid('test_log_', true) . '.log';
        \Illuminate\Support\Facades\Storage::disk('local')->put($filename, 'test log content');
        return \Illuminate\Support\Facades\Storage::disk('local')->path($filename);
    }
